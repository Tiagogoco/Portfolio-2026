...
export function Whoami() {
  ...
  return (
    <section
      id="whoami"
      ref={ref}
      className="relative h-[460vh] bg-page max-md:h-[280vh]"
    >
      <div className="sticky top-0 h-screen overflow-hidden bg-white">
        ...
        <div className="relative h-full overflow-hidden">
          <motion.div
            className="absolute left-1/2 top-[16%] whitespace-nowrap font-mono font-medium tracking-[-0.02em]"
            style={{
...
function Satellite({
  ...
  const opacity = useTransform(t, (v) =>
    clamp(Math.min(v / 0.12, (1 - v) / 0.14)),
  );
  ...
  return (
    <motion.div
      ...
      style={
        lane === "left"
          ? { left: offset, width, y, opacity }
          : { right: offset, width, y, opacity }
      }
    >
      {children}
    </motion.div>
...
function LottieSlot({ label }: { label: string }) {
  return (
    <div
      aria-label={label}
      role="presentation"
      className="aspect-square rounded-[14px] bg-lottie-slot"
    />
  );
}
